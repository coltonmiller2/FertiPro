...
export function BulkUpdatePanel({
  ...
  async function onSubmit(values: z.infer<typeof recordFormSchema>) {
    ...
    // Build only the fields that belong in Firestore (no File objects)
    const recordData: Omit<PlantRecord, "id" | "photoDataUri"> = {
    ...
    await onBulkAddRecord(plantIds, recordData, photoFile);

    // Reset form and clear the <input type="file">
    form.reset({
      date: new Date(),
      treatment: "",
      notes: "",
            ...
            <div className="p-4 space-y-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    This record will be added to all {selectedPlants.length} selected plants.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                      ...
                      <Button type="submit" className="w-full">
                        Add Record to {selectedPlants.length}{" "}
                        {selectedPlants.length === 1 ? "Plant" : "Plants"}
                      </Button>
                    </form>
                  </Form>
                </CardContent>
